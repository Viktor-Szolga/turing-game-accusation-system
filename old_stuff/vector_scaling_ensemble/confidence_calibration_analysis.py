...
N_BINS = 10
root_dir = "vector_scaling_ensemble"
run_names = ["vector_scaled_run009.pth"]
labels = ["Vector-scaled model"]
is_checkpoint = False  # Already saved as scaled model
run_names_temp = [f"run{i:03d}" for i in range(7, 300, 30)]
...
for run_name, label in zip(run_names, labels):
    ...
    specific_config = OmegaConf.load(os.path.join("early_stopping", "experiments", config_name[14:]))
    ...
    root_folder_ = "early_stopping_fixed_num_steps"
    run_names_ = [f"run{i:03d}" for i in range(7, 300, 30)]
    ...
    models = [os.path.join(root_folder_, models_dir, f"{run_name}.pth") for run_name in run_names_]
    model = EnsembleMessageClassifier(1024, [128], 2, models)
    # Wrap the base model with temperature scaling
    scaled_model = VectorScaledEnsemble(model, 2).to(config.device)
